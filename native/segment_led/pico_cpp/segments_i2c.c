...
#define BLINK_2HZ 0x2
#define BLINK_1HZ 0x4
#define BLINK_0p5HZ 0x6
...
void init()
{
    i2c_write_byte(DISPLAY_SETUP | DISPLAY_ON);
    i2c_write_byte(BRIGHTNESS);
}

// Send a specific binary value to the specified digit
static inline void display_set(int position, uint16_t bin)
{
    uint8_t buf[3];
    buf[0] = position * 2;
    buf[1] = bin & 0xff;
...
static inline void display_char(int position, char ch)
{
    display_set(position, char_to_pattern(ch));
}
...
void display_string(char *str)
{
    ...
    {
        display_char(digit++, *str++);
    }
}

void scroll_string(char *str, int interval_ms)
{
    int l = strlen(str);
...
// {
//     i2c_write_byte(BRIGHTNESS | (bright <= 15 ? bright : 15));
// }
...
void set_blink(int blink)
{
    ...
    i2c_write_byte(DISPLAY_SETUP | DISPLAY_ON | s);
}
...
#warning i2c/i2c example requires a board with I2C pins
#else
...
again:
    ...
    scroll_string("Welcome to the Raspberry Pi Pico", 150);
    ...
    display_set(0, 0xffff);
    display_set(1, 0xffff);
    display_set(2, 0xffff);
    display_set(3, 0xffff);
    ...
    set_blink(1); // 0 for no blink, 1 for 2Hz, 2 for 1Hz, 3 for 0.5Hz
    sleep_ms(5000);
    set_blink(0);
